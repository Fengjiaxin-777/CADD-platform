...
def plot_radar(props):  
    # 已将雷达图的 label 全部汉译英，防止 Matplotlib 产生中文乱码方块  
    scores = {  
        "MW Compliance": 100 if props["分子量(MW)"] <= 500 else max(10, 100 - (props["分子量(MW)"]-500)/2),  
        "LogP Compliance": 100 if props["LogP"] <= 5 else max(10, 100 - (props["LogP"]-5)*15),  
        "TPSA Fit": max(10, min(100, 100 - abs(props["极性表面积(TPSA)"]-80)/1.2)),  
        "GI Absorption": 100 if props["胃肠道吸收"] == "高吸收" else 30,  
        "Liver Safety": 100 if props["肝毒性风险"] == "较低风险" else 30,  
        "QED Index": props["QED成药性指数"] * 100  
    }  
