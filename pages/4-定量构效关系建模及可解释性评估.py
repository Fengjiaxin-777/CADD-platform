    ...
    with st.container(border=True):  
        ...
        if train_btn:  
            with st.spinner("当前分类算法参数评估学习中，请稍候..."):  
                ...
                # 已修改：将混淆矩阵的标签和轴名称转换为英文以防乱码  
                sns.heatmap(cm, annot=True, fmt="d", cmap="Blues",  
                            xticklabels=["Negative(0)", "Positive(1)"],  
                            yticklabels=["Negative(0)", "Positive(1)"], ax=ax_cm)  
    ...
    with st.container(border=True):  
        ...
        if run_comparison:  
            with st.spinner("横向预测效能对比训练中..."):  
                ...
                # 已修改：修改模型名称为纯英文，避免热图中图例(Legend)乱码  
                ...
                for name, clf in comparative_classifiers.items():  
                    ...
                    # 已修改：更改指标字典的 Keys 及其列名为英文  
                    results_records.append({  
                        "Algorithm": name,  
                        "Accuracy": round(accuracy_score(y_test, y_val_pred), 3),  
                        "Precision": round(precision_score(y_test, y_val_pred), 3),  
                        "Recall": round(recall_score(y_test, y_val_pred), 3),  
                        "F1-score": round(f1_score(y_test, y_val_pred), 3),  
                        "ROC-AUC": round(roc_auc_score(y_test, y_val_proba), 3)  
                    })  
                ...
                plot_df = compare_df.melt(id_vars="Algorithm", var_name="Metric", value_name="Score")  
                  
                fig_comp, ax_comp = plt.subplots(figsize=(10, 4.5))  
                
                # 已修改：使用纯英文列名传参给 seaborn，配置对应的 X、Y、图例和轴名称  
                sns.barplot(data=plot_df, x="Metric", y="Score", hue="Algorithm", palette="Blues_r", ax=ax_comp)  
                ax_comp.set_ylim(0, 1.1)  
                ax_comp.set_xlabel("Evaluation Metrics", fontsize=10)  
